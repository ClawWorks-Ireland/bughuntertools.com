...
## JS Bundle secrets scanning — the number that actually needs a caveat

Here's where we have to be straight with you instead of quoting a clean stat. Our tools database currently lists a "JS Bundle Analyser" skill with 7 campaigns and zero confirmed findings, which reads as unimpressive. But there's a second, higher-volume JS-scanning skill in the SecurityClaw pipeline running across all 15 campaigns with 61 real findings: hardcoded API keys, internal endpoint references, and OAuth tokens pulled straight out of minified bundles. We're reconciling internally whether these are the same capability under two names or genuinely distinct skills before we publish a combined number here, because citing 7/0 when the real figure might be 15/61 would undersell one of the highest-signal recon steps we run. Check back once that's confirmed and we'll update this section. In the meantime, the [JS bundle secrets deep-dive](/articles/js-bundle-secrets-eu-saas-recon-2026/) covers the technique itself in detail, independent of which internal label it's filed under.
